...
public final class HttpQueryUtilities extends AppCompatActivity {
    ...
    private static List<Article> extractInformationFromJson(String thisJSON) {
        ...
        try {
            ...
            // Get down to the "response" level
            JSONObject jsonResponse = jsonRawResponse.getJSONObject("response");

            // Extract the JSONArray associated with the key called "articles",
            // which represents a list of articles.
            JSONArray jsonArticleArray = jsonResponse.getJSONArray("results");
            ...
            for (int i = 0; i < jsonArticleArray.length(); i++) {
                ...
                JSONObject currentJSONArticle;
                JSONObject currentArticleJSONFields = null;

                // Get a single Article at position i within the jsonArticleArray
                currentJSONArticle = jsonArticleArray.getJSONObject(i);

                section = currentJSONArticle.getString("sectionName");
                title = currentJSONArticle.getString("webTitle");
                articleUrl = currentJSONArticle.getString("webUrl");

                try {
                    currentArticleJSONFields = currentJSONArticle.getJSONObject("fields");
                } catch (JSONException e) {
                    Log.e(LOG_TAG, "There were no fields", e);
                }
                if (currentArticleJSONFields != null) {


                    try {
                        articleImageUrl = currentArticleJSONFields.getString("thumbnail");
                    } catch (JSONException e) {
                        Log.e(LOG_TAG, "There was no thumbnail for this article", e);
                    }

                    try {
                        author = currentArticleJSONFields.getString("byline");
                    } catch (JSONException e) {
                        Log.e(LOG_TAG, "No author was credited for this article", e);
                    }

                    try {
                        description = currentArticleJSONFields.getString("trailText");
                    } catch (JSONException e) {
                        Log.e(LOG_TAG, "No author was credited for this article", e);
                    }
                }

                retrievedPublishedDate = currentJSONArticle.getString("webPublicationDate");

                // Create a new object with the args retrieved from the JSON response
                Article anObject = new Article(section, author, title, description, articleUrl, articleImageUrl, retrievedPublishedDate);
